...
// This ensures that all requests use the same single cart document.
...
// GET /api/cart - Get the contents of the common cart
export const getCart = (req, res) => {
    res.json({ items: req.cart.items });
};
...
        }
        // Fixed: Pushing only productId and quantity as per cartModel
        req.cart.items.push({
...
    try {
        await req.cart.save();
        res.status(201).json({ items: req.cart.items });
    } catch (error) {
